...
from ma_dip_rsi_buy.strategy import run_trades


def _make_df(closes: list[float], sma: list[float],
             rsi: list[float] | None = None) -> pd.DataFrame:
    """建立測試用 DataFrame，含 Close、SMA、RSI 欄位。"""
    if rsi is None:
        rsi = [30.0] * len(closes)  # 預設 RSI=30（低於常見門檻，不阻擋進場）
    return pd.DataFrame(
        {"Close": closes, "SMA": sma, "RSI": rsi},
...
class TestEntry:
    def test_entry_when_close_below_threshold(self):
        ...
        closes = [95, 89, 80, 70]
        sma = [100, 100, 100, 100]
        trades = run_trades(_make_df(closes, sma), x=5, m=10, n=30, k=15, t=10, rsi_threshold=100)
        assert len(trades) == 1
...
class TestHardStop:
    def test_hard_stop_exit(self):
        ...
        closes = [95, 90, 85, 80]
        sma = [100, 100, 100, 100]
        trades = run_trades(_make_df(closes, sma), x=5, m=6, n=30, k=10, t=15, rsi_threshold=100)
        assert len(trades) == 1
...
class TestTrailingStop:
    def test_trailing_stop_exit(self):
        ...
        trades = run_trades(_make_df(closes, sma), x=5, m=10, n=30, k=15, t=5, rsi_threshold=100)
        assert len(trades) == 1
...
class TestTimeout:
    def test_timeout_exit(self):
        ...
        sma = [100, 100, 100, 100, 100]
        trades = run_trades(_make_df(closes, sma), x=5, m=10, n=3, k=20, t=15, rsi_threshold=100)
        assert len(trades) == 1
...
class TestMACrossback:
    def test_ma_crossback_exit(self):
        ...
        closes = [95, 89, 101, 98]
        sma = [100, 100, 100, 100]
        trades = run_trades(_make_df(closes, sma), x=5, m=10, n=30, k=20, t=15, rsi_threshold=100)
        assert len(trades) == 1
...
class TestRSIFilter:
    def test_no_entry_when_rsi_above_threshold(self):
        """RSI 高於門檻時不進場，即使價格低於均線。"""
        closes = [95, 89, 80, 70]
        sma = [100, 100, 100, 100]
        rsi = [60, 55, 50, 45]  # 全部 >= 40
        trades = run_trades(
            _make_df(closes, sma, rsi),
            x=5, m=10, n=30, k=15, t=10, rsi_threshold=40,
        )
        assert len(trades) == 0

    def test_entry_when_rsi_below_threshold(self):
        """RSI 低於門檻且價格條件滿足時進場。"""
        closes = [95, 89, 80, 70]
        sma = [100, 100, 100, 100]
        rsi = [60, 25, 20, 15]  # Day 1 RSI=25 < 30
        trades = run_trades(
            _make_df(closes, sma, rsi),
            x=5, m=10, n=30, k=15, t=10, rsi_threshold=30,
        )
        assert len(trades) == 1
        assert trades[0].entry_price == 89
        assert trades[0].entry_day == 1
